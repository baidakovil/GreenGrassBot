...
CREATE TABLE IF NOT EXISTS `ggb`.`artnames` (
  ...
  `default_name` TINYINT(1) NULL COMMENT 'True if this name is used by default',
  PRIMARY KEY (`art_id`, `art_name`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `ggb`.`lineups` (
  ...
  INDEX `art_id_idx` (`art_id` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_lineups_artnames`
    FOREIGN KEY (`art_id`)
    REFERENCES `ggb`.`artnames` (`art_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `ggb`.`sentarts` (
  ...
  PRIMARY KEY (`user_id`, `event_id`, `art_id`),
  INDEX `fk_sentevents_events_idx` (`event_id` ASC) VISIBLE,
  INDEX `fk_sentevents_artnames_idx` (`art_id` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_sentevents_events`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_sentevents_artnames`
    FOREIGN KEY (`art_id`)
    REFERENCES `ggb`.`artnames` (`art_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE TABLE IF NOT EXISTS `ggb`.`lastarts` (
  ...
  INDEX `fk_lastevents_artnames_idx` (`art_id` ASC) VISIBLE,
  CONSTRAINT `fk_lastevents_users`
    FOREIGN KEY (`user_id`)
    REFERENCES `ggb`.`users` (`user_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_lastevents_artnames`
    FOREIGN KEY (`art_id`)
    REFERENCES `ggb`.`artnames` (`art_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
